...
# THESE ARE ONLY TESTS TODO TODO TODO
class InterStudyReproducibility(TemplateView):
    template_name = 'assays/test_inter.html'

    def get_context_data(self, **kwargs):
        from assays.ajax import get_inter_study_reproducibility
        context = super(InterStudyReproducibility, self).get_context_data(**kwargs)
        ...
        matrix_items = AssayMatrixItem.objects.filter(organ_model_id=24)
        ...
        inter_level = self.request.POST.get('inter_level', 1)
        max_interpolation_size = self.request.POST.get('max_interpolation_size', 2)
        initial_norm = self.request.POST.get('initial_norm', 0)

        context.update({
            'data_stuff': get_inter_study_reproducibility(
                data_points,
                matrix_items,
                inter_level,
